...
DATASETS_PATH = '/home/pablo/Proyectos/otaexpert/repos/roomimages/pycbir/static/dataset/'

def subdirs(dir):
    return [x for x in os.listdir(dir) if isdir(join(dir,x))]

DATASETS = subdirs(DATASETS_PATH)
...
class MainHandler(tornado.web.RequestHandler):
    def get(self):
        self.render('template_cbir.html', dataset=DATASETS[0], datasets=DATASETS, err_msg="", imgpath="", lists=[])
    ...
    def post(self):
        ...
        dataset = self.get_argument("dataset", "bkg_bluedoors")
        if debug_flag:
            debug = True
        else:
            debug = False
        ...
        if self.request.files:
            ...
            if extension[1:] not in EXTS:
                ...
                self.render('template_cbir.html', dataset='', datasets=[], err_msg=err_msg, imgpath=img_path, lists=[])
            ...
            phash_alg, index_alg = get_global_vars(dataset)
            lists = []
            lists += phash_alg.search(dst_full, False)
            lists += index_alg.search(dst_full, True)
        else:
            err_msg = 'No file is uploaded'
        self.render('template_cbir.html', dataset=dataset, datasets=DATASETS, err_msg=err_msg, imgpath=dst_full, lists=lists)
